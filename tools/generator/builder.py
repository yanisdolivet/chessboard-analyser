...
def create_weights(init_method, layer_size):
    match init_method:
        ...
        case "random":
            weights = [ np.random.rand(layer_size[l], layer_size[l-1]) * 0.01 for l in range(1, len(layer_size)) ]
        case _:
...
def main():
    error_args()
    filepath, nbofnn = sys.argv[1], int(sys.argv[2]) if sys.argv[2].isdigit() else print_usage() or sys.exit(1)
