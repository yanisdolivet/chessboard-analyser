...
def balance_dataset(positions, target_distribution, max_size=None):
    ...
    for state in ['Nothing', 'Check', 'Checkmate']:
        ...
        if target > avail:
            print(f"  {state:<12} {target:,} ({target_pct}%) - WARNING: Only {avail:,} available!")
            actual_count = avail
        else:
...
Example:
  ...
  ./balance_dataset.py -o balanced.txt --max-size 50000 \\
    ...
    parser.add_argument('--nothing', type=float, default=40.0,
                       help='Percentage of Nothing positions (default: 40)')
    parser.add_argument('--check', type=float, default=40.0,
                       help='Percentage of Check positions (default: 40)')
    parser.add_argument('--checkmate', type=float, default=20.0,
